...
The function should take a 2D NumPy array as input, where each row represents a data sample and each column represents a 
feature. It should return two 2D NumPy arrays: one scaled by standardization and one by min-max normalization. Make sure 
...
def feature_scaling(data: np.ndarray) -> (np.ndarray, np.ndarray):
    ...
    normalized_data = np.round((data - min_val) / (max_val - min_val) * (1 - 0) + 0, 4)
